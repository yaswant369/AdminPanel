...
function App() {
  ...
  const navItems = [
    ...
    { label: "Students", icon: "M12 12c2.21...", id: "students" },
    { label: "Companies", icon: "M3 21h18v-2...", id: "companies" },
    { label: "Projects", icon: "M10 4H4c-1.1...", id: "projects" },
    { label: "Analytics", icon: "M4 22h16v-2...", id: "analytics" },
    { label: "Mentor Reviews", icon: "M4 4h16v12...", id: "mentor" },
  ];
  ...
  return (
    <div>
      ...
      <div
        className={`app  flex min-h-[100vh] w-[20vw] bg-[#f5f7fa] text-[#1a1a1a] ${
        ...
        {isSidebarOpen && (
          <aside
            className={classNames(
              "fixed top-1/2 -translate-y-1/2 shadow-md shadow-[rgba(0,0,0,0.1)] z-50 bg-slate-800 text-white p-5 transition-all duration-300",
              "md:w-[250px] h-[80vh]",
              "w-[220px]"
            )}
            ...
            <nav className="space-y-4">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  className={classNames(
                    "flex items-center gap-3 w-full text-left p-2 rounded-md hover:bg-slate-700 transition",
                    activeSection === item.id && "bg-purple-600"
                  )}
                  onClick={() => {
